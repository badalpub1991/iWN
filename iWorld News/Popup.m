...
@interface Popup () <UITextFieldDelegate, UIGestureRecognizerDelegate, UIScrollViewDelegate,UITableViewDataSource,UITableViewDelegate,NSFetchedResultsControllerDelegate,UISearchBarDelegate/*For swiping to dimiss*/> {
    
    NSMutableArray *searcharray;
    NSMutableDictionary *myDictionary;
    NSArray *inUseDataSource;
    NSManagedObject *fetchsidename;
    NSIndexPath *indexPath1;
        UISearchBar *searchBar;
    NSArray *queryArray;
    BOOL searchfromfeed1;
    MBProgressHUD *HUD;
...
@synthesize autocompleteTableView,titleLabel,popupView;
...
-(void)setuptableview
{
    ...
    if (searchfromfeed1==YES) {
        autocompleteTableView = [[UITableView alloc] initWithFrame:CGRectMake(rect.origin.x+8,rect.origin.y+titleHeights-27, popupView.frame.size.width-16, textFieldHeight+28+100) style:UITableViewStylePlain];
        autocompleteTableView.scrollEnabled = YES;
        autocompleteTableView.hidden = YES;
        autocompleteTableView.bounces = YES;
        autocompleteTableView.backgroundColor=[UIColor clearColor];
        autocompleteTableView.dataSource=self;
        autocompleteTableView.delegate=self;
        autocompleteTableView.layer.backgroundColor=(__bridge CGColorRef)([UIColor redColor]);
        [self addSubview:autocompleteTableView];
        
    }
    else
    {
    autocompleteTableView = [[UITableView alloc] initWithFrame:CGRectMake(rect.origin.x+8, rect.origin.y+titleHeights-12, popupView.frame.size.width-16, textFieldHeight+28+28) style:UITableViewStylePlain];
    autocompleteTableView.scrollEnabled = YES;
    autocompleteTableView.hidden = YES;
    autocompleteTableView.bounces = YES;
    autocompleteTableView.backgroundColor=[UIColor clearColor];
...
- (void)formulateEverything {
    ...
    [self setupTitle];
    [self setupButton];//[self setupsearchbutton];
    NSLog(@"First execute this");
}
...
-(IBAction)sendBtnPressed:(UIButton*)sender
{
    ...
    else if ([button isEqual:_btnSearch])
    {
        searchfromfeed1 = YES;
        [_btnManual setHidden:YES];
        [_btnSearch setHidden:YES];
...
-(void)setupButton
{
    ...
    _btnManual=[[UIButton alloc]initWithFrame:CGRectMake(8, titleLabelHeight+90, popupView.frame.size.width - 16, titleLabelHeight)];
    // [btnManual setFont:[UIFont fontWithName:@"HelveticaNeue" size:kPopupTitleFontSize]];
    ...
    //  btnManual.titlel.textColor=[UIColor colorWithRed:98.0f/255.0f green:42.0f/255.0f blue:101.0f/255.0f alpha:1];
    _btnManual.backgroundColor=[UIColor clearColor];
    
    [popupView addSubview:_btnManual];
    [popupView bringSubviewToFront:_btnManual];
    
    
    
    _btnSearch=[[UIButton alloc]initWithFrame:CGRectMake(8,titleLabelHeight+150, popupView.frame.size.width - 16, titleLabelHeight)];
    // [btnManual setFont:[UIFont fontWithName:@"HelveticaNeue" size:kPopupTitleFontSize]];
...
- (void)setupTextFields {
    ...
    if (pTextFieldPlaceholderArray) {
        ...
        for (int i = 0; i < [pTextFieldPlaceholderArray count]; i++) {
            if (i == 0) {
                _textField1 = [self textFieldWithPlaceholder:pTextFieldPlaceholderArray[i] numberOfField:1];
            }
            else if (i == 1) {
                _textField2 = [self textFieldWithPlaceholder:pTextFieldPlaceholderArray[i] numberOfField:2];
            }
            else if (i == 2) {
                _textField3 = [self textFieldWithPlaceholder:pTextFieldPlaceholderArray[i] numberOfField:3];
            }
            else {
        ...
        if ([pTextFieldPlaceholderArray count] == 1) {
                       [_textField1 setFrame:CGRectMake(8, titleHeights + 24, popupView.frame.size.width - 16, textFieldHeight)];
            
            pTextFieldArray = @[_textField1];
            
            [popupView addSubview:_textField1];
        }
        else if ([pTextFieldPlaceholderArray count] == 2) {
            [_textField1 setFrame:CGRectMake(8, titleHeights + 24, popupView.frame.size.width - 16, textFieldHeight)];
            [_textField2 setFrame:CGRectMake(8, titleHeights + textFieldHeight + 32, popupView.frame.size.width - 16, textFieldHeight)];
            
            pTextFieldArray = @[_textField1, _textField2];
            
            [popupView addSubview:_textField1];
            [popupView addSubview:_textField2];
        }
        else if ([pTextFieldPlaceholderArray count] == 3) {
            
            if (searchfromfeed1==YES) {
                [_textField1 setFrame:CGRectMake(8, titleHeights +10, popupView.frame.size.width - 16, textFieldHeight)];
            ...
            [_textField1 setFrame:CGRectMake(8, titleHeights + 24, popupView.frame.size.width - 16, textFieldHeight)];
            [_textField2 setFrame:CGRectMake(8, titleHeights + textFieldHeight + 32, popupView.frame.size.width - 16, textFieldHeight)];
            [_textField3 setFrame:CGRectMake(8, titleHeights + (textFieldHeight * 2) + 40, popupView.frame.size.width - 16, textFieldHeight)];
            }
            pTextFieldArray = @[_textField1, _textField2, _textField3];
            
            [popupView addSubview:_textField1];
            [popupView addSubview:_textField2];
            [popupView addSubview:_textField3];
        }
    }
...
- (void)dismissPopup:(PopupButtonType)buttonType {
    ...
    if ([_textField1.text length]==0 && [_textField2.text length]==0 && [_textField2.text length]==0){
        NSLog(@"Text field-->1 nil");
        subTitleLabel.text=@"All Fields Mandetory";
...
//        _textField3.backgroundColor=[UIColor redColor];
    ...
    else if ([_textField1.text length]==0  || [_textField2.text length]==0  || [_textField1.text length]==0 ) {
        
        NSLog(@"All Fields are required");
        subTitleLabel.text=@"All Fields are Required";
        subTitleLabel.textColor=[UIColor redColor];
        // _textField1.backgroundColor=[UIColor redColor];
        //return;
        ...
        NSLog(@"Text field-->1 nil");
        subTitleLabel.text=@"Name is Required";
        subTitleLabel.textColor=[UIColor redColor];
      // _textField1.backgroundColor=[UIColor redColor];
        //return;
    ...
    { NSLog(@"Text field-->2 nil");
        subTitleLabel.text=@"Website Name is Required";
        subTitleLabel.textColor=[UIColor redColor];
      //  _textField2.backgroundColor=[UIColor redColor];
    ...
    { NSLog(@"Text field-->3 nil");
       subTitleLabel.text=@"Feed URL is Required";
        subTitleLabel.textColor=[UIColor redColor];
       // _textField3.backgroundColor=[UIColor redColor];

    }
        else if (_validurl==YES)
        {
            subTitleLabel.text=@"Please enter valid URL";
            subTitleLabel.textColor=[UIColor redColor];
        }
    else
    {
...
- (BOOL) urlIsValiad: (NSString *) url
{
    ...
    BOOL result = [[_textField3.text lowercaseString] hasPrefix:@"http://"];
    
    if (result) {
        return  _textField3.text;
    }
    else {
       // UITextField *textfield;
       // textfield=_textField3.text;
        NSString *good = [NSString stringWithFormat:@"http://%@", [_textField3 text]];
        _textField3.text=good;
        
    }
    NSLog(@"http included or not %@",_textField3.text);
    ...
    NSLog(@"URL is valid!%@",_textField3.text);
...
//{
//    if (_textField3) {
//        main.Feedlink=_textField3.text;
//        
//        NSLog(@"Test is working and test is %@",main.Feedlink);
//        
//        if ([self  urlIsValiad:main.Feedlink]) {
//            NSLog(@"Valid Url");
//        } else {
...
//        [self tableView:autocompleteTableView didSelectRowAtIndexPath:indexPath];
//        
//      }
    ...
    {
        _textField2.userInteractionEnabled=NO;
        //subTitleLabel.text=@"Name Must be Required";
    }
        if ([_textField2.text length]>1) {
            _textField3.userInteractionEnabled=YES;
            _textField3.backgroundColor=[UIColor whiteColor];
        }
        else
        {
            _textField3.userInteractionEnabled=NO;
           // subTitleLabel.text=@"WebsiteName must be Required";
        }
    ...
    if ([self  urlIsValiad:_textField3.text]) {
        NSLog(@"Valid Url");
        _validurl=NO;
       // successBtn.userInteractionEnabled=YES;
        
    } else {
        _validurl=YES;
      //  successBtn.userInteractionEnabled=NO;
...
replacementString:(NSString *)string
{
    ...
    const char * _char = [_textField1.text cStringUsingEncoding:NSUTF8StringEncoding];
...
- (void)searchAutocompleteEntriesWithSubstring:(NSString *)substring {
    ...
    NSFetchRequest *fetchRequest = [[NSFetchRequest alloc] initWithEntityName:@"IWorld"];
    self.searchfetch = [[managedObjectContext executeFetchRequest:fetchRequest error:nil] mutableCopy];
  ...
  //  contex=[appdelegate managedObjectContext];
    NSEntityDescription *entity = [NSEntityDescription
                                   entityForName:@"IWorld" inManagedObjectContext:managedObjectContext];
  //  NSFetchRequest *fetchRequest = [[NSFetchRequest alloc] init];
    ...
    queryArray=[NSArray new];
    
    NSError *error;
    NSPredicate *predicate=[NSPredicate predicateWithFormat:@"name contains[c] %@",substring];
    
    [fetchRequest setPredicate:predicate];
    queryArray = [managedObjectContext executeFetchRequest:fetchRequest error:&error];
...
//    
//    
//    //NSManagedObject *fetchsidename = [queryArray objectAtIndex:indexPath.row];
//
//   }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
   ...
   // [self configureCell:cell atIndexPath:indexPath];
    fetchsidename = [queryArray objectAtIndex:indexPath.row];

    [cell.textLabel setText:[NSString stringWithFormat:@"%@", [fetchsidename valueForKey:@"name"]]];
    [cell.detailTextLabel setText:[NSString stringWithFormat:@"%@", [fetchsidename valueForKey:@"feedurl"]]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
   ...
   fetchsidename = [queryArray objectAtIndex:indexPath.row];
    ...
    _textField1.text = selectedCell.textLabel.text;
    _textField2.text = [fetchsidename valueForKey:@"websitename"];
    _textField3.text = [fetchsidename valueForKey:@"feedurl"];
  
    [autocompleteTableView setHidden:YES];
    
    
//[NSString stringWithFormat:@"%@",[categoryArray objectAtIndex:indexPath.row]]];
}
